...
public class OrdemServico implements Serializable {
	...
	//esta classe é de modelo, porém também é de representação, ou seja, além de estar no pacote modelo ela retorna para a API requisitadora seus valores
	//caso eu não queira mostrar certos valores desta classe, é interessante criar uma outra classe de representação para ajustar o que pode ser visto
	
	//essas validações não são necessárias, pois agora a entrada é feita via OrdemServicoInput que já valida, se tiver outra forma de criar ordem de serviço sem passar pela input ai sim mantem
	//OrdemServicoInput nao tem o Cliente, tem apenas o Id entao nao precisa criar uma classe de validação
	...
	@ConvertGroup(from = Default.class, to = ValidationGroups.ClienteID.class) //Quando fizer essa validação, não faz a default, faz essa outra
	@ManyToOne
	@NotNull
	@JsonIgnore
	//@JoinColumn(name = ) //caso queira colocar outra coluna que não seja a padrão (ID)
